...
def auto_stop_monitor():
    """Background monitor thread to stop stream after 30 seconds of pause."""
    while True:
        time.sleep(2)
...
def fetch_rp_block(chan_id=0, bitrate=4, event=None):
    """Fetches interactive block metadata from Radio Paradise with chosen quality."""
    url = f"https://api.radioparadise.com/api/get_block?chan={chan_id}&bitrate={bitrate}&info=true"
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Channel Button with Embedded LED Layout */
        .chan-btn {
            display: flex;
            align-items: center;
